...
def plot_matrix(m: pd.DataFrame, file_name, data_path: Path, title='', subtitle='', prob=False, exclude_columns=False):
    if len(m.columns.tolist()) <= 1:
        return

    if exclude_columns:
        return plot_matrix_exclude_columns_from_heatmap(m, file_name, data_path, title, subtitle, prob)
...
def plot_matrix_exclude_columns_from_heatmap(m: pd.DataFrame, file_name, data_path: Path, title='', subtitle='', prob=False):
